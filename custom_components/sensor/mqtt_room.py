...
class MQTTRoomSensor(HomeAssistantView, Entity):
    ...
    def http_update_state(self, device_id, room, distance):
        """Update the sensor state."""
        self._state = room
        self._distance = distance
        self._updated = dt.utcnow()
    ...
    def http_message_received(self, device):
        if device.get(CONF_DEVICE_ID) == self._device_id:
            if self._distance is None or self._updated is None:
                self.http_update_state(**device)
            else:
                # update if:
                # device is in the same room OR
                # device is closer to another room OR
    ...
    def post(self, request, room):
        ...
        try:
            self.http_message_received(device)
            return self.json([])
