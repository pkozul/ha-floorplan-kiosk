...
if (typeof window.Floorplan !== 'function') {
  class Floorplan {
    ...
    init(options) {
      ...
      return ((typeof options.config === 'string') ? this.loadConfig(options.config) : Promise.resolve(options.config))
        .then(config => {
          this.config = config;
      ...
      for (let ruleInfo of entityInfo.ruleInfos) {
        for (let svgElementId in ruleInfo.svgElementInfos) {
          ...
          if (ruleInfo.rule.class_template) {
            let evaluatedClass = this.evaluate(ruleInfo.rule.class_template, entityId);
            if (!$(svgElement).hasClass(evaluatedClass)) {
              targetClass = evaluatedClass;
            }
          }
